...
- **Payments:** view-only `monero-wallet-rpc` on the relayer box, subaddress per invoice, activation at 1/2/5/10 confirmations by the invoice's USD value (under $10/$30/$100, else 10; amended 2026-09-07, was 10 flat), settled at 10 and taken back if the payment vanishes before that, renewals extend `valid_until`. XMR402 flow deferred to Stage 1 unless Ripley wants it in week 4.
...
**Status (2026-09-05).** Column A is shipped through week 4 and the weeks 5–6 drill: weeks 1–2 in `3907de6..6f5234c`, week 3 (verification, cache, majority, agreement, metrics) in `bf5625f..5a8a131`, week 4 (streamed bodies with caps, queueing, opt-out check, ejection lifecycle, load test) in `db7b11d..7551b07`, the storefront, deploy playbook, release workflow and the reorg/ejection drill after that. Column B code (storefront, token page, wallet docs, Ansible) is shipped; the human items remain: operator notices, the relayer VPS, the `.onion`, the wallet compatibility matrix, the announcement. The owned node is `node.kyc.rip` (see §10 item 7). **Beta status (2026-09-06):** the relay is live at `rpc.mnr.network` (v0.1.8), onion and I2P published, 15 upstreams, free tokens issued from the site. Wallet matrix: monero-wallet-cli, monero-wallet-gui, Feather, Cake Wallet (2026-09-07) and Monero.com (its sibling, same code) and Ripley Terminal confirmed syncing (login form, token as username); Monerujo not yet run. **Exit criteria met (2026-09-07):** a stock `monero-wallet-cli` synced from scratch to the tip through `rpc.mnr.network` (3.76M blocks, 28.7 h wall across eight legs interrupted by our own relay redeploys, 36 blocks/s, zero errors in the final leg); Feather and Cake confirmed; the drill's fake upstream is ejected with a public log entry; the owned node is on monero.fail and carried 97.5% of the stream bytes; no request logs; caps enforced by the CI load test; ten operators informed with no opt-out. Pro is on: $9 billed at a live median rate, activation tiered by value, two paid test invoices issued tokens.
...
1. **Pricing:** Pro is $9/month, Free is 500k WU/month. Confirmed; the tier table in §5 is the contract and `mnr-relay` enforces it. Amended 2026-09-07: the $9 is billed in XMR at the median of five public rate sources at invoice creation (agreement within 15%, a 30% jump held for three rounds, a 24 h stale rate refuses new invoices), replacing the hand-set XMR amount, which drifted to 3.6× the price within days. Activation tiers by USD value (1/2/5/10 confirmations under $10/$30/$100/else) with a settle phase at 10 and a take-back if the payment vanishes, same day. See `spec/storefront.md`.
